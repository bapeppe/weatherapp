...
def getWeather():
    ...
    clock.config(text="Time: " + current_time)
    name.config(text="Current Weather")

    api = "https://api.openweathermap.org/data/2.5/weather?q=" + city + "fef67063fe0d7ddf64ac61984f19625c"
    json_data = requests.get(api).json()
    condition = json_data['weather'][0]['main']
    description = json_data['weather'][0]['description']
    temp = int(json_data['main']['temp'] - 273.15)
    pressure = json_data['main']['pressure']
    humudity = json_data['main']['humidity']
...
logo_myimage = Label(image=Logo_image)
logo_myimage.place(x=150, y=100)

Frame_image = PhotoImage(file='box.png')
farme_myimage = Label(image=Frame_image)
farme_myimage.pack(padx=5, pady=5, side=BOTTOM)
